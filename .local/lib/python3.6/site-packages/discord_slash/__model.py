...
class SlashContext:
    ...
    async def send(self,
        ...
        if hidden:
            ...
            await self._http.post(base, self._discord.user.id, self.interaction_id, self.__token, not self.sent)
            self.sent = True
            return
        send_type = 2 if send_type == 3 else 5 if send_type == 4 else send_type
        print(send_type)
        if (send_type == 2 or send_type == 5) and not self.sent:
            ...
            self.sent = True
        #webhook = Webhook.from_url(f"https://discord.com/api/v8/webhooks/{self._discord.user.id}/{self.__token}",
...
class SlashWebhookAdapter(WebhookAdapter):
    BASE = "https://discord.com/api/v8"

    def __init__(self, bot: typing.Union[discord.Client, discord.AutoShardedClient, commands.Bot, commands.AutoShardedBot]):
        self.bot = bot
