...
class ask(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        qamount = '.ask how many'
        channel4get = self.client.get_channel(217142893440270346)

        meme = ['Nemesis#2441']
        if str(message.author) in meme:
            if str.lower(qamount) in message.content:
                dict = {
                    1: '1',
                    2: '10',
                    3: '1 000 000',
                ...
                await channel.send(str(dict[intanswer]))
                return

            if message.content.startswith('.ask When'):
                dict = {
                    1: 'Tomorrow',
                    2: 'Today',
                    3: 'Never',
